...
public class JmmAnalysisImpl implements JmmAnalysis {
    ...
    private List<AnalysisVisitor> buildPasses(SymbolTable table) {
        return List.of(
                ...
                new InvalidBinaryExpr(table),
                new InvalidUnaryExpr(table),
                new InvalidArrayExpr(table),
                new InvalidConditionStmt(table),
                new InvalidAssignStmt(table),
                new InvalidThisExpr(),
                new InvalidExprStmt(),
                new IncompatibleReturn(table),
                new IncompatibleArgs(table),
                new UninitializedVariable(table)
        );
    }
    ...
    public JmmSemanticsResult semanticAnalysis(JmmSemanticsResult semanticsResult) {
        ...
        for (var analysisVisitor : analysisVisitors) {
            try {
                ...
                hasWarnings |= passReports.stream()
                        .anyMatch(report -> report.getType() == ReportType.WARNING);
        ...
        if (hasWarnings) {
            System.out.println("Found warnings: " + reports);
        }
